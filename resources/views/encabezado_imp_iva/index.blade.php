...
<div class="container">
{!! Form::open(['route' => 'encabezado_imp_iva.index', 'method'=>'GET', 'Class'=>'navbar-form navbar-right']) !!}
...
<h4><b><center>REGISTROS DE encabezado_imp_iva</h4></b></center>
<a class="btn btn-info" data-toggle="modal" href='#crear_encabezado_imp_iva'><span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Crear encabezado_imp_iva</a>

  @include('encabezado_imp_iva.create')
...
<th>fecha_auditoria_encabezado_imp_iva</th>
<th>fecha_elaboracion</th> -->
  ...
  @foreach($encabezado_imp_iva as $row)
    <tr>
          ...
          <td>{{$row->fecha_auditoria_encabezado_imp_iva}}</td>
          <td>{{$row->fecha_elaboracion}}</td> -->

         <td>
              <a class="btn bg-navy btn-flat margin" href="{{ $url = route('detalle_imp_iva.edit', $row->id) }}" role="button"><i class="fa fa-list-alt" aria-hidden="true"></i> Detalle Impuesto IVA</a>  
          </td>

          <td><a data-toggle="modal" data-target="#editar_encabezado_imp_iva" 
          data-id="{{$row->id}}"
...
<center>{{ $encabezado_imp_iva->links() }}</center>
